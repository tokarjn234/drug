...
class OpenSSLCert extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'OpenSSLCert {quantity?}';
    ...
    public function handle()
    {
        ...
        if (empty ($quantity)) {
            $quantity = $this->ask('Enter the quantity of certificates ');
        }
    ...
    private function createCert()
    {
        ...
        $newCer->save();
        DB::commit();
        $this->info('Create new client certificate success. New ssl_client_s_dn_cn=' . $commonName);
    }
}
